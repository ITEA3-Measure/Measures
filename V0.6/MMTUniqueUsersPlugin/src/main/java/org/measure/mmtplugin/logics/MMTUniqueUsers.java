...
public class MMTUniqueUsers extends MMTLogic{
            ...
            */
            ...
            MongoCursor<Document> cursorb = collection.aggregate(
                    Arrays.asList(
                            ...
                            Aggregates.group("$18", Accumulators.sum("IPCount", 1))
                    )
            ).iterator();
            ...
            while(cursorb.hasNext()){
                ...
                ips.put(doc.getString("_id"), doc.getInteger("IPCount"));
            }
            
            measurement.addValue("totalUsers", ips.keySet().size());
            measurement.addValue("IPList", ips.keySet().toString());
            
            measurement.setValue(ips.keySet().size());
